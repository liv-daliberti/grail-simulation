...
set -euo pipefail
trap 'rc=$?; echo "[evaluate-grpo-wage] failure (exit ${rc}) at line ${LINENO}" >&2' ERR
...
echo "[evaluate-grpo-wage] $(date --iso-8601=seconds) • starting evaluation; logs -> $LOG_DIR" >&2
...
echo "[evaluate-grpo-wage] $(date --iso-8601=seconds) • launching python -m grpo.pipeline" >&2
