...
try:  # pragma: no cover - optional dependency
    from .utils.callbacks import PushToHubRevisionCallback
...
@dataclass(frozen=True)
class GrpoPipelineRuntime:
    """Runtime configuration for the GRPO pipeline.

    :param dataset: Mapping containing train and evaluation splits.
...
@dataclass(frozen=True)
class GrpoPipelineInputs:
    """Parameter bundle forwarded to :func:`execute_grpo_pipeline`.

    :param component_factory: Factory used to construct pipeline components.
    :param reward_funcs: Sequence of reward functions applied during training.
    :param tokenizer: Tokenizer instance used by the trainer.
    :param runtime: Runtime configuration capturing dataset and argument state.
    :param evaluate_fn_factory: Optional factory producing evaluation wrappers.
    """
    ...
    reward_funcs: Sequence[Any]
    tokenizer: Any
    runtime: GrpoPipelineRuntime
    evaluate_fn_factory: Optional[EvalFnFactory] = None

    @property
    def dataset(self) -> Mapping[str, Any]:
        """Return the dataset mapping carried by :class:`GrpoPipelineRuntime`.

        :returns: Dataset mapping defined by the runtime configuration.
        """

        return self.runtime.dataset

    @property
    def script_args(self) -> Any:
        """Return script arguments carried by :class:`GrpoPipelineRuntime`.

        :returns: Script argument namespace defined by the runtime configuration.
        """

        return self.runtime.script_args

    @property
    def training_args(self) -> Any:
        """Return training arguments carried by :class:`GrpoPipelineRuntime`.

        :returns: Training argument namespace defined by the runtime configuration.
        """

        return self.runtime.training_args

    @property
    def model_args(self) -> Any:
        """Return model arguments carried by :class:`GrpoPipelineRuntime`.

        :returns: Model argument namespace defined by the runtime configuration.
        """

        return self.runtime.model_args

    @property
    def logger(self) -> Any:
        """Return logger instance carried by :class:`GrpoPipelineRuntime`.

        :returns: Logger instance defined by the runtime configuration.
        """
    ...
    @property
    def prefix(self) -> str:
        """Return logging prefix carried by :class:`GrpoPipelineRuntime`.

        :returns: Logging prefix defined by the runtime configuration.
        """
...
def execute_grpo_pipeline(
    *,
    inputs: GrpoPipelineInputs,
) -> Tuple[Any, Tuple[Any, Optional[Mapping[str, Any]]]]:
    """
    Assemble components via ``inputs.component_factory`` and execute the shared GRPO pipeline.

    :param inputs: Bundle containing component factory, data, and configuration.
    :returns: Tuple mirroring :func:`prepare_model_eval_and_run_grpo`.
    """

    components = inputs.component_factory.build(
        reward_funcs=inputs.reward_funcs,
        tokenizer=inputs.tokenizer,
        evaluate_fn_factory=inputs.evaluate_fn_factory,
    )
    context = build_grpo_context(
        inputs.dataset,
        inputs.script_args,
        inputs.training_args,
        inputs.model_args,
        inputs.logger,
...
_GRPO_OPTIONAL_KEYS = {"evaluate_fn_factory"}
_GRPO_REQUIRED_KEYS = {
    "component_factory",
    "reward_funcs",
    "tokenizer",
...
def make_grpo_execute_kwargs(
    ...
    **overrides: Any,
    ...
    """
    Return a :class:`GrpoPipelineInputs` instance for :func:`execute_grpo_pipeline`.

    Helper used by entrypoints to avoid repeating lengthy keyword lists while
    retaining the flexibility to override individual parameters.

    :param prefix: Logging prefix propagated to downstream helpers.
    :param namespace: Mapping used to infer missing pipeline parameters.
    :param overrides: Explicit keyword overrides, e.g. ``dataset`` or ``tokenizer``.
    :returns: Populated :class:`GrpoPipelineInputs` bundle.
    :raises ValueError: If required pipeline parameters are missing.
    """

    if namespace is not None:
        payload: Dict[str, Any] = dict(collect_grpo_pipeline_kwargs(namespace))
    else:
        payload = {}

    unknown_keys = set(overrides) - (_GRPO_REQUIRED_KEYS | _GRPO_OPTIONAL_KEYS)
    if unknown_keys:
        raise ValueError(f"Unsupported GRPO override keys: {sorted(unknown_keys)}")
    ...
    if "evaluate_fn_factory" not in payload:
        payload["evaluate_fn_factory"] = None

    missing = [
        key
        for key in _GRPO_REQUIRED_KEYS
        if key not in payload or payload[key] is None
    ]
    if missing:
        raise ValueError(
            "Missing GRPO pipeline parameters: "
            + ", ".join(sorted(missing))
        )
    ...
    runtime = GrpoPipelineRuntime(
        ...
        training_args=payload["training_args"],
        model_args=payload["model_args"],
        logger=payload["logger"],
        prefix=payload["prefix"],
    )

    return GrpoPipelineInputs(
        component_factory=payload["component_factory"],
        reward_funcs=payload["reward_funcs"],
        tokenizer=payload["tokenizer"],
        runtime=runtime,
        evaluate_fn_factory=payload.get("evaluate_fn_factory"),
    )
