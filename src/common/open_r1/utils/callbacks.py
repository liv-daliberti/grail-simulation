...
class SuccessCachingCallback(TrainerCallback):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, replay_buffer: ReplayBuffer, acc_threshold: float = 0.999):
        ...
        :param replay_buffer: Buffer that stores high-quality prompts.
        :param acc_threshold: Minimum accuracy required to cache a prompt.
        :returns: ``None``. Stores references for later callbacks.
        """
        super().__init__()
    ...
    def set_trainer(self, trainer):                  # called once at start
        ...
        :param trainer: Trainer instance exposing ``_textual_logs``.
        :returns: ``None``. Saves the trainer reference.
        """
    ...
    def on_log(
        ...
        **_kwargs,
        ...
        :param _args: Training arguments (unused).
        :param _state: Trainer state (unused).
        :param _control: Trainer control object (unused).
        :param _logs: Latest scalar logs emitted by the trainer.
        :param _kwargs: Additional keyword arguments from the trainer.
        :returns: ``None``. Adds prompts to the buffer when thresholds are met.
        """
...
class ReplayBufferCallback(TrainerCallback):  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        ...
        :param replay_buffer: Buffer storing prompts that exceed the threshold.
        :param tokenizer: Tokenizer used to decode prompt token ids.
        :param accuracy_key: Reward head name providing accuracy scores.
        :param threshold: Minimum reward required to enqueue a prompt.
        :returns: ``None``. Prepares the callback for training hooks.
        """
        super().__init__()
        self.buf  = replay_buffer
        self.tok  = tokenizer
    ...
    def on_train_batch_end(self, args, _state, _control, **batch_kwargs):
        ...
        :param args: Training arguments providing runtime metadata.
        :param _state: Trainer state (unused).
        :param _control: Trainer control object (unused).
        :param batch_kwargs: Dictionary containing ``inputs`` and ``outputs``.
        :returns: ``None``. Adds prompts to the replay buffer when eligible.
        """
    ...
    def _maybe_store_prompt(self, accuracy: float, token_ids) -> int:
        ...
        :param accuracy: Reward score for the prompt.
        :param token_ids: Token ids forming the prompt input.
        :returns: ``1`` when the prompt was stored, otherwise ``0``.
        """
    ...
    def _log_batch_stats(self, args, replay_flags, batch_size: int, added: int) -> None:
        ...
        :param args: Training arguments namespace (used for rank detection).
        :param replay_flags: Flags indicating replay-originated samples.
        :param batch_size: Size of the processed batch.
        :param added: Number of prompts added to the buffer.
        :returns: ``None``. Emits structured logs.
        """
