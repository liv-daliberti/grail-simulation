#!/bin/bash
#SBATCH --job-name=GRAIL_GUN
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=64
#SBATCH --mem=256G
#SBATCH --time=12:00:00
#SBATCH --output=logs/grail_gun/slurm_%j.out
...
LOG_DIR=${LOG_DIR:-"$ROOT_DIR/logs/grail_gun"}
RUN_NAME=${RUN_NAME:-Qwen1.5B-GRAIL-GUN}
TIMESTAMP=${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}

CONFIG=${CONFIG:-"$ROOT_DIR/recipes/Qwen2.5-1.5B-Instruct/grpo/config_grail_gun.yaml"}
ACCEL_CONFIG=${ACCEL_CONFIG:-"$ROOT_DIR/recipes/accelerate_configs/zero3.yaml"}
MAIN_SCRIPT=${MAIN_SCRIPT:-"$ROOT_DIR/src/grail/grail.py"}
export CONFIG
...
for dir in "$HF_HOME" "$HF_HUB_CACHE" "$HF_DATASETS_CACHE" "$XDG_CACHE_HOME" \
