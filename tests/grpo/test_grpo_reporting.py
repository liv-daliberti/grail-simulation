...
"""Regression tests for GRPO pipeline reporting outputs."""
import json
...
def test_generate_reports_writes_markdown(tmp_path):
    ...
    catalog = repo_root / "reports" / "grpo" / "README.md"
    next_readme = repo_root / "reports" / "grpo" / "next_video" / "README.md"
    opinion_readme = repo_root / "reports" / "grpo" / "opinion" / "README.md"
    ...
    assert "GRPO Next-Video Baseline" in next_readme.read_text(encoding="utf-8")
    assert "GRPO Opinion Regression" in opinion_readme.read_text(encoding="utf-8")


def test_filter_selection_allows():
    selection = FilterSelection.from_raw(issues=("Gun_Control",), studies=("Study1",))
    allowed = selection.allows(
        SimpleNamespace(issue="Gun_Control", participant_study="Study1")
    )
    blocked_issue = selection.allows(
        SimpleNamespace(issue="Minimum_Wage", participant_study="Study1")
    )
    blocked_study = selection.allows(
        SimpleNamespace(issue="Gun_Control", participant_study="Study3")
    )

    assert allowed is True
    assert blocked_issue is False
    assert blocked_study is False
...
def test_pipeline_reports_stage_reads_cached_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(grpo_pipeline, "_repo_root", lambda: tmp_path)
    ...
    summary_catalog = tmp_path / "reports" / "grpo" / "README.md"
