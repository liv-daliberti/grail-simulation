#!/bin/bash
#SBATCH --job-name=GRAIL_GRPO_GUN
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=64
#SBATCH --mem=256G
#SBATCH --time=12:00:00
#SBATCH --output=logs/grpo_gun/slurm_%j.out
...
LOG_DIR=${LOG_DIR:-"$ROOT_DIR/logs/grpo_gun"}
RUN_NAME=${RUN_NAME:-Qwen1.5B-GRPO-GUN}
TIMESTAMP=${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}

CONFIG=${CONFIG:-"$ROOT_DIR/recipes/Qwen2.5-1.5B-Instruct/grpo/config_grpo_gun.yaml"}
